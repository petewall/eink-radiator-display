...
"""Steps related to the display command"""
...
def step_impl(context):
    context.process = subprocess.run(['pipenv', 'run', 'python', 'main.py', 'display', 'test/pencils.jpg', '--save', 'output.png'], capture_output=True, text=True)
...
def step_impl(_):
    with Image.open('output.png') as image:
        image.save('test/pencils_quantized.png')
        with Image.open('test/pencils_quantized.png') as expected:
